...
const ForecastCapsule = ({
  ...
  const timeDateOpacityDisplay=():[string,number]=>{
    let opacity=0;
    let timeOrDay="";
    if(type===ForecastType.Hourly){timeOrDay=convertDateTo12HrFormat(date);
      opacity=timeOrDay.toLowerCase()==="now"?1:0.2;    }else{
...
const[timeToDisplay,capsuleOpacity]=timeDateOpacityDisplay();
  ...
  const probabiltyOpacity=probability>0?1:0;
  return (
    <View style={{ width: width, height: height }}>
      ...
      >
        ...
        <View>
          ...
          />
          <Text style={[styles.probability,{opacity:probabiltyOpacity}]}>{probability}%</Text>
        </View>
