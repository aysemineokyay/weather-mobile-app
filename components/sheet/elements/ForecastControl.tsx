...
const ForecastControl = ({ onPress }: ForecastControlProps) => {
  ...
  const animatedSegmentLineStyles = useAnimatedStyle(() => {
    return { transform: [{ translateX: segmentTranslateX.value }] };
  });
  const onForecastPress = (type: ForecastType) => {
    if (type === ForecastType.Weekly) {
      segmentTranslateX.value = withTiming(width - textWidth - spaceingX * 2, {
        duration: 1000,
      });
    } else {
  ...
  return (
    <>
      ...
      <AnimatedCanvas
        style={[
          { height: strokeWidth, width: textWidth, marginLeft: spaceingX },
